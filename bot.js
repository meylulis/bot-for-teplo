...
// Обработка выбора челленджа "Искусство в словах"
bot.hears('🖋️ Искусство в словах', (ctx) => {
    const userId = ctx.from.id;

    // Очищаем статус пользователя, чтобы избежать конфликтов
    if (!usersData[userId]) {
        usersData[userId] = {};
    }

    usersData[userId].status = 'waiting_for_words'; // Новый статус для текста
    saveData(usersData);

    ctx.reply('🖋️ Для участия в челлендже напишите сообщение из 20 слов о выставке. Пример: "Эта выставка впечатляет своими уникальными произведениями искусства..."');
});
...
bot.on('text', (ctx) => {
    ...
    // Если пользователь не участвует в челлендже "Искусство в словах", пропускаем
    if (!userState || userState.status !== 'waiting_for_words') {
        return;
    }

    const userMessage = ctx.message.text.trim();
    const wordCount = userMessage.split(/\s+/).length; // Разделяем сообщение на слова и считаем их

    // Проверяем, что в сообщении ровно 20 слов
    if (wordCount === 20) {
        // Начисляем 10 баллов
        usersData[userId].points = (usersData[userId].points || 0) + 10;
        saveData(usersData);
        ...
        // Отправляем пользователю сообщение о том, что челлендж выполнен
        ctx.reply(
            `🎉 Вы успешно выполнили челлендж "Искусство в словах"! 🖋️\n\n` +
            `Вы получили *10 баллов*! 💰\n\n` +
            `💰 Ваш текущий баланс: *${usersData[userId].points}* баллов.`,
            { parse_mode: 'Markdown' }
        );
    } else {
        // Если в сообщении не 20 слов, информируем пользователя
        ctx.reply('⚠️ Ваше сообщение должно содержать ровно 20 слов. Пожалуйста, попробуйте еще раз!');
    }
});
