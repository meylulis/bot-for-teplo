...
bot.on('text', (ctx) => {
    ...
    // Если пользователь не участвует в текстовом челлендже, не обрабатываем текст
    if (!userState || !userState.status) {
        return;
    }

    // Проверяем, что текстовый челлендж активен
    if (userState.status === 'waiting_for_vitrail_story') {
        const wordCount = messageText.split(/\s+/).length; // Подсчет слов
