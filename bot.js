...
// Обработчики для каждого челленджа
const challenges = [
...
bot.on('photo', (ctx) => {
    ...
    // Проверка на статус "ожидание фото" для Кружков и Мероприятий или Челленджей
    if (userState && userState.status) {
        ...
        // Если статус соответствует одному из челленджей
        if (pointsEarned > 0) {
            // Начисляем баллы
            ...
            // Если статус не соответствует ожиданию фото для челленджа или кружка/мероприятия
