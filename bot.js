...
// Обработка новых мероприятий
bot.hears('Выставка "Я так вижу"', (ctx) => {
    const userId = ctx.from.id;
    if (!usersData[userId]) {
        usersData[userId] = { points: 0 };
    }

    usersData[userId].activity = 'Выставка "Я так вижу"';
    saveData(usersData);

    ctx.reply('Вы выбрали мероприятие "Выставка "Я так вижу". Отправьте два фото с выставки и получите 5 баллов!');
});

bot.hears('Сильная трапеция', (ctx) => {
    const userId = ctx.from.id;
    if (!usersData[userId]) {
        usersData[userId] = { points: 0 };
    }

    usersData[userId].activity = 'Сильная трапеция';
    saveData(usersData);

    ctx.reply('Вы выбрали мероприятие "Сильная трапеция". Запишите видео, как вы пробуете упражнение на трапеции, и получите 5 баллов!');
});

bot.hears('Витражная мастерская', (ctx) => {
    const userId = ctx.from.id;
    if (!usersData[userId]) {
        usersData[userId] = { points: 0 };
    }

    usersData[userId].activity = 'Витражная мастерская';
    saveData(usersData);

    ctx.reply('Вы выбрали мероприятие "Витражная мастерская". Пришлите фото вашего витражного изделия и получите 5 баллов!');
});

bot.hears('Керамические посиделки', (ctx) => {
    const userId = ctx.from.id;
    if (!usersData[userId]) {
        usersData[userId] = { points: 0 };
    }

    usersData[userId].activity = 'Керамические посиделки';
    saveData(usersData);

    ctx.reply('Вы выбрали мероприятие "Керамические посиделки". Поделитесь историей о своей работе и получите 5 баллов!');
});

bot.hears('Арт-терапевтическая встреча', (ctx) => {
    const userId = ctx.from.id;
    if (!usersData[userId]) {
        usersData[userId] = { points: 0 };
    }

    usersData[userId].activity = 'Арт-терапевтическая встреча';
    saveData(usersData);

    ctx.reply('Вы выбрали мероприятие "Арт-терапевтическая встреча". Пришлите фото вашего арт-терапевтического рисунка и получите 5 баллов!');
});

bot.hears('Теплые мастерские', (ctx) => {
    const userId = ctx.from.id;
    if (!usersData[userId]) {
        usersData[userId] = { points: 0 };
    }

    usersData[userId].activity = 'Теплые мастерские';
    saveData(usersData);

    ctx.reply('Вы выбрали мероприятие "Теплые мастерские". Поделитесь аудио-сообщением о том, кому вы хотите подарить свою работу, и получите 5 баллов!');
});

// Обработка отправки фото для новых челленджей
bot.on('photo', (ctx) => {
    const userId = ctx.from.id;
    const activity = usersData[userId]?.activity;

    if (!activity) {
        ctx.reply('⚠️ Сначала выберите мероприятие или кружок!');
        return;
    }
    ...
    switch (activity) {
        case 'Выставка "Я так вижу"':
            pointsAwarded = 5;
            ctx.reply('Фото принято! Вы получили 5 баллов за участие в челлендже "Выставка "Я так вижу"!');
            break;
        case 'Витражная мастерская':
            pointsAwarded = 5;
            ctx.reply('Фото принято! Вы получили 5 баллов за участие в челлендже "Витражная мастерская"!');
            break;
        default:
            ctx.reply('Фото принято! Вы получили 10 баллов!');
            break;
    }

    // Начисление баллов
    usersData[userId].points += pointsAwarded;
    saveData(usersData);

    ctx.reply(`💰 Ваш текущий баланс: *${usersData[userId].points}* баллов`, { parse_mode: 'Markdown' });
...
// Обработка текстовых сообщений (для других челленджей, например, текстовых отзывов)
bot.on('text', (ctx) => {
    const userId = ctx.from.id;
    const activity = usersData[userId]?.activity;
    ...
    switch (activity) {
        case 'Керамические посиделки':
            pointsAwarded = 5;
            ctx.reply('Спасибо за ваш отзыв! Вы получили 5 баллов за участие в челлендже "Керамические посиделки"!');
            break;
        case 'Арт-терапевтическая встреча':
            pointsAwarded = 5;
            ctx.reply('Спасибо за ваш отзыв! Вы получили 5 баллов за участие в челлендже "Арт-терапевтическая встреча"!');
            break;
        case 'Теплые мастерские':
            pointsAwarded = 5;
            ctx.reply('Спасибо за ваш голос! Вы получили 5 баллов за участие в челлендже "Теплые мастерские"!');
            break;
        default:
            ctx.reply('Спасибо за участие! Вы получили 10 баллов!');
            break;
    }
...
bot.hears('🎨 Рисуй каждый день!', (ctx) => {
    ...
    ctx.reply('🎨 Поучаствуй в челлендже! Отправляй мне любые свои рисунки каждый день и заработай баллы.');
});
...
bot.on('photo', (ctx) => {
    ...
    if (!usersData[userId] || !usersData[userId].activity) {
        ctx.reply('⚠️ Сначала выберите кружок или мероприятие перед отправкой фото!');
        return;
    }

    // Начисляем 10 баллов за посещение кружка/мероприятия
    usersData[userId].points = (usersData[userId].points || 0) + 10;

    // Сохраняем данные
    saveData(usersData);

    // Отправляем пользователю сообщение о начисленных баллах
    ctx.reply(
