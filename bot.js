...
activities.forEach(activity => {
    bot.hears(activity, (ctx) => {
        ...
        // Убедимся, что данные пользователя существуют
        if (!usersData[userId]) {
            usersData[userId] = { points: 0 }; // Если данных нет, создаем их
        }
        ...
        ctx.reply(
            `📷 Вы выбрали *${activity}*.\n` +
            `Отправьте в течение этого мероприятия/кружка фотографию и получите *10 баллов*!`,
            { parse_mode: 'Markdown' });
...
bot.on('photo', (ctx) => {
    ...
    // Начисление 10 баллов за фото
    const currentPoints = userState.points || 0; // Получаем текущие баллы, если они есть
    ...
    // Очищаем сохраненный кружок/мероприятие, чтобы пользователь мог выбрать следующее
