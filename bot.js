...
bot.hears('🖋️ Искусство в словах', (ctx) => {
    ...
    usersData[userId].status = 'waiting_for_review'; // Новый статус
    saveData(usersData);

    ctx.reply('📝 Для участия в челлендже отправьте короткий отзыв о выставке. Вы получите за это 5 баллов.');
});

// Обработка отправки текста в рамках челленджа "Искусство в словах"
bot.on('text', (ctx) => {
    const userId = ctx.from.id;
    const userState = usersData[userId];

    // Проверяем, что пользователь участвует в челлендже "Искусство в словах"
    if (!userState || userState.status !== 'waiting_for_review') {
        return;
    }

    // Получаем текст отзыва
    const review = ctx.message.text.trim();

    // Если отзыв не пустой, начисляем баллы
    if (review.length > 0) {
        // Начисляем 5 баллов
        userState.points = (userState.points || 0) + 5;
        saveData(usersData);
        ...
        // Сообщаем пользователю, что он выполнил челлендж
        ctx.reply(
            `✅ Вы успешно выполнили челлендж "Искусство в словах"! 🎉\n\n` +
            `Вы получили *5 баллов*! 💰\n\n` +
            `💰 Ваш текущий баланс: *${userState.points}* баллов.`,
            { parse_mode: 'Markdown' }
        );
    } else {
        // Если отзыв пустой, просим пользователя повторить
        ctx.reply('⚠️ Отзыв не может быть пустым! Пожалуйста, отправьте короткий отзыв о выставке.');
    }
});
...
bot.hears('🔙 Вернуться назад', (ctx) => {
    ...
    // Очистим статус пользователя, если он участвует в каком-либо челлендже
    if (usersData[userId]) {
        delete usersData[userId].status; // Удаляем статус, если он есть
        saveData(usersData); // Сохраняем изменения
    }
