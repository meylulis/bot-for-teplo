...
bot.on('text', (ctx) => {
    ...
    // Проверяем, что пользователь находится в процессе отправки текста для челленджа
    if (userState && userState.status === 'waiting_for_vitraj_idea') {
        const userText = ctx.message.text.trim();

        if (userText.split(' ').length >= 20) {
            // Начисляем 10 баллов за участие в челлендже
            usersData[userId].points = (usersData[userId].points || 0) + 10;
            saveData(usersData);

            // Отправляем сообщение о начисленных баллах
            ctx.reply(
        ...
        // Очищаем статус пользователя после обработки
