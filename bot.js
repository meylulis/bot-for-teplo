...
// Обработка выбора кружков
function handleActivitySelection(ctx, activityType) {
    activities.forEach(activity => {
        bot.hears(activity, (ctx) => {
            const userId = ctx.from.id;

            // Если данных о пользователе нет, создаем их
            if (!usersData[userId]) {
                usersData[userId] = { points: 0 }; // Начальные данные
            }

            // Сохраняем выбранную активность
            usersData[userId].activity = { type: activityType, name: activity };
            saveData(usersData);

            // Сообщение о том, что выбран кружок или мероприятие
            ctx.reply(
                `📷 Вы выбрали *${activity}*.\n` +
                `Отправьте в течение этого ${activityType} фотографию и получите *10 баллов*!`,
                { parse_mode: 'Markdown' }
            );
        });
    });
...
bot.on('photo', (ctx) => {
    ...
    // Проверяем, было ли выбрано мероприятие
    if (userState?.activity?.type !== 'мероприятие') {
        return; // Игнорируем фото, если не выбрано мероприятие
    }

    // Начисляем 10 баллов за фото
    const currentPoints = userState.points || 0; // Получаем текущие баллы
    const newPoints = currentPoints + 10; // Добавляем 10 баллов

    // Обновляем данные пользователя
    usersData[userId].points = newPoints;
    saveData(usersData);

    // Отправляем сообщение с подтверждением и текущим балансом
    ctx.reply(
    ...
    // Очищаем сохраненную активность
...
bot.on('photo', (ctx) => {
    ...
    // Проверяем, было ли выбрано кружок
