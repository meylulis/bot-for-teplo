...
bot.hears('💡 История витража', (ctx) => {
    ...
    // Устанавливаем статус пользователя для ожидающего текст
    usersData[userId].status = 'waiting_for_vitraj_idea';
    saveData(usersData);

    ctx.reply('📝 Для участия в челлендже "История витража" напишите 20 слов об идее вашего витражного изделия.');
});

// Обработка выбора челленджа "История витража"
bot.hears('💡 История витража', (ctx) => {
    const userId = ctx.from.id;
    ...
    usersData[userId].status = 'waiting_for_vitraj_idea'; // Устанавливаем статус ожидания текста
    saveData(usersData);

    ctx.reply('📝 Напишите 20 слов об идее вашего витражного изделия для участия в челлендже "История витража".');
});

// ОБРАБОТКА ТОЛЬКО ЕСЛИ ПОЛЬЗОВАТЕЛЬ В СТАТУСЕ "waiting_for_vitraj_idea"
bot.on('text', (ctx) => {
    const userId = ctx.from.id;
    const userState = usersData[userId];

    if (userState && userState.status === 'waiting_for_vitraj_idea') {
        ...
        if (userText.split(' ').length >= 20) {
            ...
            ctx.reply(
                `✅ Ваше описание принято! Вы получили *10 баллов* за участие в челлендже "История витража"! 🎉\n\n` +
                `💰 Ваш текущий баланс: *${usersData[userId].points}* баллов.`,
                { parse_mode: 'Markdown' }
            );
        } else {
            ctx.reply('❌ Ваше описание должно содержать минимум 20 слов. Пожалуйста, отправьте более подробное описание.');
            return; // Не снимаем статус, даём возможность отправить заново
        }

        delete usersData[userId].status; // Сбрасываем статус
        saveData(usersData);
    }
});
