...
// Обработка выбора кружка и мероприятия
const activities = [
...
// Обработка отправки фото (объединение с начислением баллов)
bot.on('photo', (ctx) => {
    const userId = ctx.from.id;
    const userState = usersData[userId];
    ...
    // Если пользователь выбрал мероприятие или кружок
    if (userState.activity) {
        // Начисляем 10 баллов за фото с мероприятия/кружка
    ...
    // Проверяем достижения
    checkAchievements(userId, userState.activity);
